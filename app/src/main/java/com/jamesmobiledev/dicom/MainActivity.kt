...
import com.jamesmobiledev.dicom.databinding.ActivityMainBinding
...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val sourceFile = File(
            this.cacheDir.absolutePath, "chest.jpg"
        )
...
//        DicomGenerator(jpgFile, destinationFile)

        openAsset(destinationFile.toUri())
...
//                ?.filter { it.type == "application/dicom" }
    ...
    /* private fun openAsset(data: Uri) {
         try {
             ...
             binding.patientView.text = loadDataSet.getPatientName(
                 TagId(0x10, 0x10),
                 0,
    ...
    private fun openAsset(data: Uri) {
        try {
            ...
            // The usage of the Pipe allows using files on Google Drive or other providers
            val imebraPipe = PipeStream(32000)

            // Launch a separate thread that reads from the InputStream and pushes the data
            // to the Pipe.
            val pushThread = Thread(PushToImebraPipe(imebraPipe, stream!!))
            pushThread.start()

            // The CodecFactory will read from the Pipe, which is fed by the thread launched
            // before. We could just pass a file name to it, but this would limit what we
            // can read to only local files
            val loadDataSet = CodecFactory.load(StreamReader(imebraPipe.streamInput))

            // Log DICOM file data
            logDicomFileData(loadDataSet)
...
//            Log.d("@@@", "openAsset: ${loadDataSet.getString(TagId(0, Tag.PatientBirthDate), 0)}")
        } catch (e: Exception) {
    ...
    private fun logDicomFileData(dataSet: DataSet) {
        Timber.tag("DicomLogger").d("DICOM File Data:")
        Timber.tag("DicomLogger").d(
            "Patient Name: ${
                dataSet.getPatientName(
                    TagId(0x10, 0x10),
                    0,
                    PatientName("Undefined", "", "")
                ).alphabeticRepresentation
...
//        Timber.tag("DicomLogger").d("Patient Name: ${dataSet.getString(Tag., )}")
//        Timber.tag("DicomLogger").d("Study Description: ${dataSet.getString(Tag.StudyDescription, "N/A")}")
        // Add more log statements for other DICOM attributes as needed
    }
